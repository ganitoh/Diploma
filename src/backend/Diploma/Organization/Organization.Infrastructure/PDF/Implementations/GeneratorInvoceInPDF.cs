...
public class GeneratorInvoceInPDF : IGenerateInvoiceForOrder
{
    ...
    foreach (var item in order.Items)
    {
        ...
        gfx.DrawString(item.Name ?? "-", normalFont, XBrushes.Black,
            new XRect(col1 + 5, tableY + 5, 225, rowHeight), XStringFormats.TopLeft);

        gfx.DrawString(item.Quantity.ToString(), normalFont, XBrushes.Black,
            new XRect(col2, tableY, 80, rowHeight), XStringFormats.Center);

        gfx.DrawString("шт.", normalFont, XBrushes.Black,
            new XRect(col3, tableY, 80, rowHeight), XStringFormats.Center);

        gfx.DrawString($"{item.PriceUnit.Value:N2}", normalFont, XBrushes.Black,
            new XRect(col4, tableY, 80, rowHeight), XStringFormats.Center);

        gfx.DrawString($"{item.TotalPrice.Value:N2}", normalFont, XBrushes.Black,
            new XRect(col5, tableY, 90, rowHeight), XStringFormats.Center);
    ...
    // Итог
    gfx.DrawString($"ИТОГО: {order.TotalPrice:N2} руб.",
        titleFont, XBrushes.Black,
        new XRect(0, tableY, page.Width - leftMargin, 30),
    ...
    gfx.DrawLine(XPens.Black, leftMargin + 70, tableY + 12,
        leftMargin + 70 + signatureLineWidth, tableY + 12);

    gfx.DrawString("Покупатель", normalFont, XBrushes.Black, col3, tableY);
    gfx.DrawLine(XPens.Black, col3 + 90, tableY + 12,
        col3 + 90 + signatureLineWidth, tableY + 12);
